...
**Architecture:** `scripts/retired GAPS lift script` reads a skill package directory and produces a v1 spec draft using the fingerprint and generated `implementation.v1.yml` when present, plus heuristic recovery of spec structure when not. `scripts/retired GAPS round trip script` drives `generate → lift → semantic-diff` and exits non-zero on any structural divergence. The round-trip diff is structural: it compares the dehydrated spec to the lifted draft after removing fields the lift cannot recover (free-text fields like `accountabilityScope` text). Two new user-facing skills — `retired GAPS lift` and `retired GAPS round trip` — give human users the round-trip discipline through `/gaps:lift` and `/gaps:round-trip`. Docs live under `docs/governed-autonomy/gaps/`. v0.1 tooling stays in place but emits a one-line deprecation notice on invocation.
...
- `scripts/retired v1 lift package/__init__.py`
- `scripts/retired v1 lift package/recover.py`
- `scripts/retired v1 lift package/diff.py`
- `tests/gaps/v1/test_lift.py`
- `tests/gaps/v1/test_round_trip.py`
- `skills/retired GAPS lift/SKILL.md`
- `skills/retired GAPS lift/agents/openai.yaml`
- `skills/retired GAPS round trip/SKILL.md`
- `skills/retired GAPS round trip/agents/openai.yaml`
...
**Modified files:**
- `scripts/retired v1 generator package/implementation_map.py` — add top-level gate and evidence ids to the v1 implementation binding when required for round-trip recovery
...
- `skills/retired GAPS author/SKILL.md` — point at v1 author guidance
- `skills/retired GAPS validate/SKILL.md` — point at v1 conformance levels
- `skills/retired GAPS generate/SKILL.md` — point at v1 generator
- `scripts/retired GAPS v0 validator` — print one-line deprecation notice
- `scripts/retired GAPS package builder` — print one-line deprecation notice
...
**Files:**
- Create: `scripts/retired v1 lift package/__init__.py`
- Create: `scripts/retired v1 lift package/recover.py`
- Create: `scripts/retired GAPS lift script`
...
```bash
mkdir -p scripts/retired v1 lift package
touch scripts/retired v1 lift package/__init__.py
```
...
from retired v1 lift package.recover import lift_package, LiftError  # noqa: E402
...
```bash
chmod +x scripts/retired GAPS lift script
python3 scripts/retired GAPS lift script gaps/examples/v1/benefits-eligibility-review/expected
```
...
```bash
git add scripts/retired v1 lift package scripts/retired GAPS lift script
...
**Files:**
- Create: `scripts/retired v1 lift package/diff.py`
- Create: `scripts/retired GAPS round trip script`
- Modify if the smoke test exposes missing structural binding: `scripts/retired v1 generator package/implementation_map.py`
...
from retired v1 lift package.diff import diff_skeletons, lift_skeleton, spec_skeleton  # noqa: E402
from retired v1 lift package.recover import LiftError, lift_package  # noqa: E402
from gaps_v1_validator.loader import load_yaml  # noqa: E402
...
def main() -> int:
    ...
    with tempfile.TemporaryDirectory() as tmp:
        result = subprocess.run(
            [sys.executable, str(REPO_ROOT / "scripts" / "retired GAPS v1 package builder"),
...
```bash
chmod +x scripts/retired GAPS round trip script
python3 scripts/retired GAPS round trip script gaps/examples/v1/benefits-eligibility-review/ga-process.v1.yml
```
...
```bash
git add scripts/retired v1 lift package/diff.py scripts/retired GAPS round trip script
...
LIFT_SCRIPT = ROOT / "scripts" / "retired GAPS lift script"
...
```python
"""Tests for retired GAPS round trip."""
...
ROOT = Path(__file__).resolve().parents[3]
SCRIPT = ROOT / "scripts" / "retired GAPS round trip script"
...
**Files:**
- Create: `skills/retired GAPS lift/SKILL.md`, `skills/retired GAPS lift/agents/openai.yaml`
- Create: `skills/retired GAPS round trip/SKILL.md`, `skills/retired GAPS round trip/agents/openai.yaml`
...
```bash
mkdir -p skills/retired GAPS lift/agents skills/retired GAPS round trip/agents commands/gaps
```

- [ ] **Step 2: Write `skills/retired GAPS lift/SKILL.md`**

```markdown
---
name: retired GAPS lift
...
\`\`\`bash
python3 scripts/retired GAPS lift script <package-root>
\`\`\`

Pass `--out <path.yml>` to write to a file instead of stdout.
```

- [ ] **Step 3: Write `skills/retired GAPS lift/agents/openai.yaml`**
...
- [ ] **Step 4: Write `skills/retired GAPS round trip/SKILL.md`**

```markdown
---
name: retired GAPS round trip
...
\`\`\`bash
python3 scripts/retired GAPS round trip script <spec>
\`\`\`
```

- [ ] **Step 5: Write `skills/retired GAPS round trip/agents/openai.yaml`**
...
skill = "retired GAPS round trip"
path = "skills/retired GAPS round trip"
```
...
```bash
ls skills/retired GAPS lift skills/retired GAPS round trip
...
```bash
git add skills/retired GAPS lift skills/retired GAPS round trip commands/gaps/lift.md commands/gaps/lift.toml commands/gaps/round-trip.md commands/gaps/round-trip.toml agent-skills.json gemini-extension.json
git commit -m "Add retired GAPS lift and retired GAPS round trip skills and commands"
```
...
**Files:**
- Modify: `skills/retired GAPS author/SKILL.md`
- Modify: `skills/retired GAPS validate/SKILL.md`
- Modify: `skills/retired GAPS generate/SKILL.md`

- [ ] **Step 1: Update `skills/retired GAPS author/SKILL.md`**
...
- [ ] **Step 2: Update `skills/retired GAPS validate/SKILL.md`**
...
- [ ] **Step 3: Update `skills/retired GAPS generate/SKILL.md`**
...
Generate v1 skill packages with `python3 scripts/retired GAPS v1 package builder <spec>`. The spec must be at `conformanceLevel: generative`. Pass `--validate-after` to confirm before generation.

After generation, verify the package with `python3 scripts/retired GAPS round trip script <spec>`.
...
```bash
git add skills/retired GAPS author/SKILL.md skills/retired GAPS validate/SKILL.md skills/retired GAPS generate/SKILL.md
git commit -m "Point retired GAPS author, retired GAPS validate, retired GAPS generate skills at v1"
```
...
**Files:**
- Modify: `scripts/retired GAPS v0 validator`
- Modify: `scripts/retired GAPS package builder`
- Modify: `gaps/README.md`

- [ ] **Step 1: Add deprecation banner to `scripts/retired GAPS v0 validator`**

Read the file. Locate the `main()` function. Insert at the top of `main()`:

```python
import sys
sys.stderr.write("NOTE: scripts/retired GAPS v0 validator validates GAPS v0.1 specs. v1.0.0 is the active version — see scripts/validate-gaps-v1.py.\n")
```

- [ ] **Step 2: Add deprecation banner to `scripts/retired GAPS package builder`**

Locate `main()`. Insert at the top:

```python
import sys
sys.stderr.write("NOTE: scripts/retired GAPS package builder generates v0.1 skeletons. v1.0.0 is the active version — see scripts/retired GAPS v1 package builder.\n")
```
...
- `scripts/retired GAPS v0 validator` — v0.1 validator (frozen).
- `scripts/retired GAPS package builder` — v0.1 generator (frozen).

v0.1 reference specs continue to validate against the v0.1 validator. A
migrator (`scripts/retired v0-to-v1 migration tool`) produces a v1 spec at
...
```bash
git add scripts/retired GAPS v0 validator scripts/retired GAPS package builder gaps/README.md
...
Re-validate. Then round-trip: `python3 scripts/retired GAPS round trip script <spec>`.
...
\`\`\`bash
python3 scripts/retired GAPS v1 package builder <spec> [--output-root <dir>] [--validate-after]
\`\`\`
...
\`\`\`bash
python3 scripts/retired v0-to-v1 migration tool gaps/examples/<process>/ga-process.yml
\`\`\`
